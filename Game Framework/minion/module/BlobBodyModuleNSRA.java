...
public abstract class BlobBodyModuleNSRA extends MinionModule{
	...
	//Create The Chain
	//Add Fluff
	public BlobBodyModuleNSRA(MinionEntity owner, float main_radius) {
		super(owner);
...
//			djd.initialize(main, b, main.getWorldCenter().add(nextpos.mul(main_radius)), b.getWorldCenter().sub(nextpos.mul(bodyRadius)));
			...
			djd.initialize(main, b, main.getWorldCenter().add(nextpos.mul(main_radius)), b.getWorldCenter());
...
//					:) Thanks Roy
	...
	public boolean doAI() {
		for(int i=0;i<bodies.size();i++){
			Body b = bodies.get(i);
			float ref_ang = MathUtils.PI*i*2f/(float)bodies.size();
			float curang = b.getAngle()+b.getAngularVelocity()/60-main.getAngle();
			if(curang != 0){
				float speed = (1-squishdamp*squishfreq/60)*(b.getAngularVelocity()-main.getAngularVelocity()) + squishfreq/60*curang;
				b.applyAngularImpulse((main.getAngle()-curang)*60*(b.getInertia()));
			}
