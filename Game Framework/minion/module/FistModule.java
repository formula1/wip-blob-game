...
public abstract class FistModule extends MinionModule{
...
//	CollisionManager punchboo; 
	...
	public FistModule(MinionEntity owner, Body connection, float chestwidth) {
...
//			The_Game.jhelp.tweenB(pushes, pushes.getLowerLimit()+pushlim_len*.1f);
	...
	private void swipe(float direction){
		...
		if(!ready){
			The_Game.jhelp.tweenB(pushes, pushes.getUpperLimit());
			The_Game.jhelp.rtweenB(rotates, direction-(float)Math.PI/4f);
			if(	pushes.getJointTranslation() >= pushes.getUpperLimit()-pushlim_len*.1f
			&&	(	rotates.getJointAngle() >= direction-(float)Math.PI/4f-Math.PI/12
				&&	rotates.getJointAngle() <= direction-(float)Math.PI/4f+Math.PI/12
				)
			) ready = true;
			...
			The_Game.jhelp.rtweenB(rotates, direction+(float)Math.PI/4f);
			if(	(	rotates.getJointAngle() >= direction+(float)Math.PI/4f-Math.PI/12
				&&	rotates.getJointAngle() <= direction+(float)Math.PI/4f+Math.PI/12
				)
			){
				ready = false;
